...
namespace Jong2DTest
{
    ...
    public static class Collision
    {
        ...
        {
            public ICollidable From { get; set; }
            public CollisionEvent(ICollidable from) : base(SDL.SDL_EventType.SDL_FIRSTEVENT)
            {
                this.From = from;
            }
        }
        public static void AABBCollision(ICollidable from, ICollidable to)
        {
            if (from == to)
            {
                return;
            ...
            if (from.GetBB().Collide(to.GetBB()))
            {
                IGameObject obj = to as IGameObject;
                if (obj != null)
                {
                    obj.EventHandle(new CollisionEvent(from), 0);
                }
            }
        }
    }
    ...
    public class Grass : IGameObject, ICollidable
    {
        static Image image;
        Vector2D pos;
        Size2D size;
        ...
        {
            return BoundingBox.Create(this.pos, size);
        }
    }
    ...
    public class Ball : IGameObject, ICollidable
    {
        ...
        static Size2D Size = new Size2D(42, 42);
        int dir = -1;
        static Ball()
        {
            Ball.image = Context.LoadImage(@"Resources\ball41x41.png");
        ...
        public void EventHandle(GameEvent e, double frame_time)
        {
            if (e is Collision.CollisionEvent)
            {
                var ev = e as Collision.CollisionEvent;
                if (ev.From is Grass)
                {
                    dir = 0;
                }
        ...
        public void Update(double frame_time)
        {
            double distance = 200 * frame_time;
            double y = Pos.y + dir * distance;
            Pos.y = y;
        }
    }
